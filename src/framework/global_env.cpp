...
void Global::InitEnv() {
    nlohmann::json envJson = this->GetJsonObject("env/env_path.json");
    nlohmann::json settingsJson = this->GetJsonObject("env/settings.json");
    ...
    for (const auto& keyValue : settingsJson.items()) {
        const std::string& key = keyValue.key();
        const nlohmann::json& value = keyValue.value();
        this->envPaths[key] = value;
    }
...
nlohmann::json Global::GetJsonObject(const std::string& jsonPath){
...
void Global::InitAssets() {
    ...
    assetsMgr->LoadShader("final", "assets/shaders/final.vsh", "assets/shaders/final.fsh");
    // 加载噪声图
    assetsMgr->LoadTexture("perlin/noise", "assets/textures/perlinNoise.png");
    // 加载模型
